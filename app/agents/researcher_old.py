...
mock_database = MockSearchDatabase()
...
def researcher_agent(state: InvestigationState) -> InvestigationState:
    ...
    Implements multi-hop investigation using tool interfaces.
    """
    logger.info("=" * 60)
    ...
    # Execute the task using appropriate tool
    try:
        search_result = execute_task(next_task, discovered_entities, mock_database, evidence_gaps)
        ...
        logger.error(f"RESEARCHER - Error executing task '{next_task.description}': {e}")
...
def filter_duplicate_documents(
    ...
    Filter out documents that are already in the existing documents list.
    """
    existing_ids = {doc["document_id"] for doc in existing_documents}
...
def generate_follow_up_searches(
    documents: List[Dict[str, Any]],
    entities: Dict[str, Any]
) -> List[str]:
    """
    Generate follow-up search suggestions based on newly discovered information.
    """
    suggestions = []
    
    # Extract new services from documents
    services = set()
    for doc in documents:
        if doc.get("service"):
            services.add(doc["service"])
    
    # Extract new incident IDs
    incident_ids = set()
    for doc in documents:
        if doc.get("incident_id"):
            incident_ids.add(doc["incident_id"])
    ...
    for doc in documents:
        ...
        if "database" in content:
            key_terms.add("database")
        if "api" in content:
            key_terms.add("api")
        if "connection" in content:
            key_terms.add("connection")
        if "timeout" in content:
            key_terms.add("timeout")
    ...
    for incident_id in incident_ids:
        suggestions.append(f"incident_id:{incident_id}")
